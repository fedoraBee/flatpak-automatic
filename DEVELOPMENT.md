...
7. Sync the content to `repo/rpms/latest/<CHANNEL>/` and `repo/debs/latest/<CHANNEL>/`.

### Hosting on GitHub

To host this as a repository on GitHub:

1. The `repo` structure is staged into `public/` and deployed to the root
   of the `gh-pages` branch alongside the `index.html` landing page by the CI
   workflow.
2. The `repo/` contains `rpms/` and `debs/` subdirectories for each package type.
3. The GPG public key is available as `gpt.key` at the root of the `gh-pages` branch.
4. Users can then add the repository by creating a `.repo` or `.list` file pointing to the
