...
class TestAutomationEngineStatus:
    ...
    @patch("flatpak_automatic.notifiers.DesktopNotifier.is_available")
    @patch("flatpak_automatic.notifiers.WebhookNotifier.is_available")
    def test_print_status_overview_verbose(
        self,
        mock_wh: MagicMock,
        mock_dt: MagicMock,
        mock_mail: MagicMock,
        ...
        mock_run.side_effect = [
            ...
            MagicMock(returncode=1),  # is-active
            MagicMock(
                stdout="App\torg.test.App\t1.0\tstable\tsystem\n", returncode=0
        ...
        engine = AutomationEngine({"exclusions": ["org.test.App"]}, {})
        with patch("builtins.print") as mock_print:
            engine.print_status_overview(verbose=True)
            printed_args = [
                call.args[0] if call.args else "" for call in mock_print.call_args_list
            ]
            assert any(
                "📦 Installed Flatpaks & Runtimes (Verbose):" in arg
                for arg in printed_args
            )
            assert any("APPLICATION ID" in arg for arg in printed_args)
            assert any("(Excluded)" in arg for arg in printed_args)
