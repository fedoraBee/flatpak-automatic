...
try:
    ...
    print("Warning: python3-dbus is not installed. Snapper snapshots will be bypassed.", file=sys.stderr)
    ...
    def __init__(self, config="root"):
        ...
        if DBUS_AVAILABLE:
            try:
                ...
                self.proxy = self.bus.get_object('org.opensuse.Snapper', '/org/opensuse/Snapper')
                self.interface = dbus.Interface(self.proxy, 'org.opensuse.Snapper')
            except Exception as e:
                print(f"Warning: Failed to connect to Snapper DBus interface: {e}", file=sys.stderr)
...
class MailNotifier:
    ...
    def send_mail(self, subject, body):
        if not self.mail_cmd or not self.to_address:
            ...
            return
        try:
            process = subprocess.Popen([self.mail_cmd, '-s', subject, '-r', self.from_address, self.to_address], stdin=subprocess.PIPE)
...
def main():
    ...
    if trigger_notify:
        ...
        mailer = MailNotifier(
            ...
            config.get('FLATPAK_MAIL_FROM', f"flatpak-automatic@{socket.gethostname()}")
        )
        mailer.send_mail(
